...
export const SocialPostWriterTool: React.FC<SocialPostWriterToolProps> = ({ onBackToDashboard }) => {
    ...
    const generateSocialPost = useAction(api.generations.generateSocialPost);
    const generateBlogHeroImage = useAction(api.generations.generateBlogHeroImage);
    ...
    }, [inputType, inputValue, postType, tone, length, generateSocialPost]);
    ...
    const handleGenerateHeroImage = useCallback(async () => {
        ...
        try {
            const imageUrl = await generateBlogHeroImage({ content: generatedContent });
    ...
    }, [generatedContent, generateBlogHeroImage]);
