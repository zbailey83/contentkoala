...
const CreditsDisplay: React.FC = () => {
    ...
    if (user === undefined) {
        return <div className="h-6 w-16 bg-gray-200 dark:bg-secondary-accent rounded-md animate-pulse" />;
    }
    
    return (
        <div className="px-4 py-2 bg-gray-100 dark:bg-secondary-accent/50 rounded-lg">
            <span className="text-sm font-semibold text-gray-800 dark:text-primary-text">{user?.credits ?? 0} Credits</span>
        </div>
    );
};
  ...
  return (
    <aside className="w-64 bg-white dark:bg-background border-r border-gray-200 dark:border-secondary-accent flex-shrink-0 flex-col hidden sm:flex">
      ...
      <nav className="flex-1 px-4 py-4 space-y-2">
        ...
        />
        <NavLink
          icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M3 4a1 1 0 00-1 1v10a1 1 0 001 1h14a1 1 0 001-1V5a1 1 0 00-1-1H3zm3 2a1 1 0 011-1h8a1 1 0 110 2H7a1 1 0 01-1-1zm0 4a1 1 0 011-1h8a1 1 0 110 2H7a1 1 0 01-1-1z" /></svg>}
          label="Buy Credits"
          isActive={activeTool === 'payments'}
          onClick={() => onNavClick('payments')}
        />
      </nav>
      <div className="mt-auto p-4 border-t border-gray-200 dark:border-secondary-accent">
        <NavLink
