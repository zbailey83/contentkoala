...
interface VideoGeneratorProps {
  ...
  onGenerate: () => void;
  disabled: boolean;
  sourceImageId: Id<"_storage"> | null;
...
const VideoOutput: React.FC<{ generation: VideoGeneratorProps['generation'] }> = ({ generation }) => {
    ...
    if(generation?.status === 'completed' && generation.url) {
        ...
        return (
            <div className="w-full flex flex-col gap-4 items-center">
                <video src={generation.url} controls autoPlay loop className="max-h-[400px] w-full object-contain rounded-lg bg-gray-200 dark:bg-background" />
...
const SourceImage: React.FC<{ storageId: Id<"_storage"> | null }> = ({ storageId }) => {
    const imageUrl = useQuery(api.files.getUrl, storageId ? { storageId } : 'skip');
    ...
    return (
        <div className="relative w-32 h-32 bg-gray-100 dark:bg-background/30 rounded-lg flex items-center justify-center border border-gray-300 dark:border-secondary-accent">
            {imageUrl ? (
                <img src={imageUrl} alt="Video source preview" className="max-h-full max-w-full object-contain rounded-md" />
            ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col space-y-4">
                    <div>
                        ...
                        <SourceImage storageId={sourceImageId} />
                        {isCustomSource && (
                    ...
                    <div>
                     ...
                     <div className="pt-2">
                        <Button onClick={onGenerate} disabled={disabled}>
                            {props.generation?.status === 'pending' ? 'Generating...' : '🎬 Generate Video Ad'}
                        </Button>
                    </div>
                </div>
                ...
                <div className="w-full h-full flex flex-col items-center justify-center bg-gray-100 dark:bg-secondary-accent/50 rounded-lg p-4 min-h-[300px]">
                    <VideoOutput generation={props.generation} />
                </div>
            </div>
        </div>
    );
